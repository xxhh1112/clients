...
class OverlayButton {
  ...
  init() {
    const overlayButtonRoot = document.getElementById("overlay-button-root");
    this.shadowDom = overlayButtonRoot.attachShadow({ mode: "closed" });
    ...
    this.buttonElement.addEventListener("click", () => {
      chrome?.runtime?.sendMessage({ command: "openAutofillOverlayList" });
    });
  }
}

const bitwardenOverlayButton = new OverlayButton();
bitwardenOverlayButton.init();
