...
To make a row selectable with a checkbox, add a cell with `bit-row-selector` and pass the row's data
to `bitRow`.

```html
<tr [bitRow]="row" *ngFor="let r of rows$ | async">
  <td bitCell bit-row-selector></td>
  <td bitCell>{{ r.name }}</td>
</tr>
```

Adding `bit-row-selector` to a table header will render a select all checkbox:

```html
<tr>
  <th bit-row-selector></th>
  <th bitCell>Name</th>
</tr>
```

The table's `DataSource.selectionModel` exposes a
[CDK SelectionModel](https://material.angular.io/cdk/collections/overview) that contains the
selected rows.
...
<Story of={stories.SelectableRows} />
